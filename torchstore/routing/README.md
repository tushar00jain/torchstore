...
- Spawn `RoutingCoordinator` - everyone will send their tensor layout to this coordinator. It constructs and forwards the routing table or the layouts as is to generators. If layouts are forwarded, generators build the routing table.
...
- Users call `register_state_dict` or `register_state_dict_locally` - these say where the routing table gets built
  - Both send user provided `state_dict` to coordinator `RoutingCoordinator`
  - `register_state_dict_locally` calls `RoutingCoordinator.register_layouts()` that tells coordinator to not build anything but send the layouts of everyone back
  - `register_state_dict` calls `RoutingCoordinator.register()` that tells coordinator to build the routing table and send it back
- After client has the routing table, it calls `RoutingDirectory.install(...)` which stores this local routing table
- To build the routing table, client calls `RoutingPlan.build_for()`, coordinator calls `RoutingPlan.build()`

```
ts.client(role=...)
        │
RoutingClient.register_state_dict(...)
        │
        ├──▶ RoutingCoordinator.register(...) ──▶ RoutingPlan.build(...)
        │                                             │
